...
//        enemy_bar.setAttribute("style","width:"+String(new_width)+"px");
    ...
    show_grid_blocker: function(color, text) {
        ...
        if (color) {
            ...
            // Enemy turn — show red blocker with clear message
            grid_blocker.style.display = "block";
            grid_blocker.style.backgroundColor = "rgba(120,0,0,0.85)";
            document.getElementById("block_grid_text").innerHTML = "⚔️ " + (Model.enemy ? Model.enemy.name : "Enemy") + "'s Turn!";
        }
    },

    hide_grid_blocker: function() {
        var grid_blocker = document.getElementById("block_grid");
        grid_blocker.style.display = "none";
        document.getElementById("block_grid_text").innerHTML = "";
    },

    show_turn_banner: function(whose_turn, name) {
        var banner = document.getElementById("turn_banner");
        if (!banner) { return; }
        banner.className = "turn-banner " + whose_turn;
        banner.textContent = whose_turn === "player" ? "✏️ Your Turn!" : "⚔️ " + name + "'s Turn!";
        banner.style.display = "block";
        // auto-hide after 1.8s
        clearTimeout(View._banner_timeout);
        View._banner_timeout = setTimeout(function() {
