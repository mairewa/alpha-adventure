var Controller = {
    ...
    respond_to_tile_click: function() {
        if (this.innerHTML != "" && Model.whoseturn == "player") {
            ...
            Learn.highlight_blanks(Model.tiles.selected_tiles);
        }
    },
    ...
    checkword: function(word) {
        var _this = this;
        WordCheck.ready.then(function() {
            if (WordCheck.is_valid(word)) {
                Learn.record_word(word);
    ...
    special_checkword: function(word, category, executeme) {
        WordCheck.is_valid_in_category(word, category, function(valid) {
            if (valid && executeme) { executeme(); }
        });
    },
...
// negative player effects        
    ...
    enemy_attack_sequence: function() {
        View.show_turn_banner("enemy", Model.enemy.name);
        document.getElementById("enemy_name").classList.add("active-turn");
        document.getElementById("player_name").classList.remove("active-turn");
...
//stun
        ...
        if (p_old_health == Model.player.hp && e_old_health == Model.enemy.hp) {
            ...
            View.hide_grid_blocker();
            View.show_turn_banner("player", Model.player.name);
            document.getElementById("player_name").classList.add("active-turn");
            document.getElementById("enemy_name").classList.remove("active-turn");
        } else {
    ...
    player_death: function() {
        ...
        setTimeout(() => { window.location.replace("youdied.html"); }, 2700)
    },
    ...
    enemy_defeated: function() {
        ...
        if (finishpage) {
            ...
            setTimeout(() => { Learn.show_recap(); }, waittime);
            setTimeout(() => { window.location.replace(finishpage+".html"); }, waittime + 6000);
        } else if (Model.enemy.id.slice(-1) == "1") {
            setTimeout(() => { Learn.show_recap(); }, waittime);
            setTimeout(() => { window.location.replace("map.html"); }, waittime + 6000);
        }    else {
            setTimeout(() => { Learn.show_recap(); }, waittime);
            setTimeout(() => { this.next_enemy(); }, waittime + 6000);
        }
